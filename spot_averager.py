...
class Spot_Averager(QtCore.QObject):
    ...
    def __init__(self, num_frames: int = 1, camera: CameraController = None):
        ...
        print(f"Spot Averager Instance created with camera: {camera}")

        if camera:
            self.camera = camera
            print("connecting camera to spot averager")
            print(f"Signal: {self.camera.worker.spot_tracker.focused_com_update}")
